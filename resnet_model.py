...
class ResNet(object):
    ...
    def _build_model(self):
        ...
        for i in range(1, self.hps.num_residual_units):
            with tf.variable_scope('unit_1%d' % i):
    ...
    def _build_train_op(self):
        ...
        if self.hps.optimizer == 'sgd':
            optimizer = tf.train.GradientDescentOptimizer(self.lrn_rate, 0.9)
        else:
    ...
    def _conv(self, name, x, filter_size, in_filters, out_filters, strides):
        with tf.variable_scope(name):
            ...
            kernel = tf.get_variable('Weights', [filter_size, filter_size,
                                                 in_filters, out_filters], tf.float32,
